...
class EditPost extends AbstractAccount implements CsrfAwareActionInterface, HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        if ($validFormKey && $this->getRequest()->isPost()) {
            ...
            $currentPhoneNumber = $customer->getCustomAttribute('mp_sms_telephone')
                ? $customer->getCustomAttribute('mp_sms_telephone')->getValue()
                : null;

            $newPhoneNumber = $this->getRequest()->getParam('telephone');

            $customerCandidate = $this->populateNewCustomerDataObject($this->_request, $customer);

            if ($currentPhoneNumber !== $newPhoneNumber) {
                $customerCandidate->setCustomAttribute('mp_sms_telephone', $newPhoneNumber);
            }
            ...
            try {
                ...
                if ($currentPhoneNumber !== $newPhoneNumber) {
                    $customer->setCustomAttribute('mp_sms_telephone', $newPhoneNumber);
                }
                ...
                // Logout from the current session if password or email changed
            ...
            // Save post data in session to repopulate the form if needed
