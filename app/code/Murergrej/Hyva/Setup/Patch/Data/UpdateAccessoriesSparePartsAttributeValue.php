...
class UpdateAccessoriesSparePartsAttributeValue implements DataPatchInterface
{
    /**
    ...
    private function insertProductValue($entityId)
    {
        ...
        $valueReservedele   = $this->getRawValue($entityId, 'reservedele');
        $valueTilbehor = $this->getRawValue($entityId, 'tilbehor');

        // Concatenate values and set new value for 'about'
        if (empty($valueReservedele) && empty($valueTilbehor)) {
            $newValue = '';
        } else {
            $newValue = sprintf('%s%s', $valueReservedele, $valueTilbehor);

        }

        // Get the attribute ID for 'about'
        $attributeId = $this->getAttributeId('accessories_spare_parts');
