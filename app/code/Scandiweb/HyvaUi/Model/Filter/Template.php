...
namespace Scandiweb\HyvaUi\Model\Filter;
...
/**
 * Overridden to:
 * - Add lazy load feature (#lazy-load-comment)
 *
 * {@inheritDoc}
 */
class Template extends SourceTemplate
{
    ...
    protected function changeImageSrc(DOMDocument $document) : void
    {
        ...
        foreach ($nodes as $node) {
            ...
            if (!$node->hasAttribute('data-skip-lazyload')) {
                if ($src->nodeValue !== '') {
                    $classes = '';
                    ...
                    // Using loading="lazy" attribute for Safari browser due to
                    // issue with lazy loading images.
                    $isSafari ? $node->setAttribute('loading', 'lazy') : $node->removeAttribute('src');
                }
            }
        }
    }
